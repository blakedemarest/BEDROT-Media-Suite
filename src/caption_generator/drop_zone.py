...
class DropZoneWidget(QFrame):
    ...
    def _validate_audio_files(self, urls) -> list:
        """
        Filter URLs to only include supported audio files.
    ...
    def dragEnterEvent(self, event: QDragEnterEvent):
        ...
        if event.mimeData().hasUrls():
            # Check if any files are valid audio files
            valid_files = self._validate_audio_files(event.mimeData().urls())
            if valid_files:
                event.acceptProposedAction()
                self._drag_active = True
                self._update_style()
                self.label.setText(f"Drop {len(valid_files)} audio file(s)")
                return
